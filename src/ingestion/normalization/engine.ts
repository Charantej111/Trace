...
export interface NormalizationEngineContext {
  workspaceId: string;
  sourceId: string;
  importId: string;
...
export class NormalizationEngine {
  public static normalizeBatch(
    ...
    context: NormalizationEngineContext
  ): { records: CanonicalFeedback[]; validCount: number; duplicateCount: number; invalidCount: number } {
    ...
    rows.forEach(row => {
      ...
      if (mappings.rating && data[mappings.rating] !== undefined) {
      ...
      // Capture all unmapped headers into normalizedMetadata
      ...
      const fingerprint = extId
        ...
        : FingerprintEngine.generateContentFingerprint(context.workspaceId, originalText, custEmail || custName);
